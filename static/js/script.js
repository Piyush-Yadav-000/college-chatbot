...
function appendMessage(sender, text) {
    ...
    if (sender === "user") {
        ...
        msgContainer.style.opacity = "0"; // Start hidden for animation
...
function sendMessage(predefined = null) {
    ...
    const thinkingTime = 600 + Math.random() * 400; // 600-1000ms
    ...
    body: JSON.stringify({ msg: userText })
        ...
        .then(data => {
            ...
            // Add slight delay before showing bot response for natural feel
            setTimeout(() => {
                if (data.response) {
...
function toggleChat() {
    ...
    const isMinimized = container.style.transform === "scale(0)" || container.style.display === "none";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Add send button click sound effect (optional)
    if (sendBtn) {
...
// Clear chat function (optional - can be triggered by a button)
function clearChat() {
    const chatBox = document.getElementById("chat-box");
...
window.addEventListener("load", () => {
    ...
    // Check if chat should start minimized (optional)
    const startMinimized = false; // Set to true if you want chat minimized by default
